    ...
    try {
      ...
      for (const id of uniqueIDs) {
        try {
          ...
          // Use fallback mapping instead of GetNameFromTypeValue
...
export default function IFCViewer({
  ...
  // Explode effect
  useEffect(() => {
    if (!components || components.length === 0 || originalCentroids.length === 0) return;

    const overallCenter = new THREE.Vector3();
    originalCentroids.forEach((cent) => overallCenter.add(cent));
    overallCenter.divideScalar(originalCentroids.length);
    ...
    components.forEach((component, i) => {
      ...
      const vector = new THREE.Vector3().subVectors(centroid, overallCenter);
      const size = mesh.geometry.boundingBox.getSize(new THREE.Vector3()).length();
      const scale = size * 0.1;
      mesh.position.copy(vector.multiplyScalar(explodeAmount * scale));
    });
